...
class VideoContentRepository {
    ...
    async createVideoContent(videoContentData: VideoContent): Promise<VideoContent> {
    ...
    async updateVideoContentById(videoContentId: string, videoContentData: VideoContent): Promise<VideoContent> {
    ...
    async getVideoContents(query: any): Promise<VideoContent[]> {
        const { title, originTitle, releaseYears, ratingRange, genres, actors, directors, lists, sort, fields } = query;
        ...
        if (releaseYears) {
            const years: number[] = releaseYears.split(',').map((year: string) => parseInt(year.trim()));
        ...
        if (ratingRange) {
            const [minRating, maxRating] = ratingRange.split(',').map((rating: string) => parseFloat(rating.trim()));
