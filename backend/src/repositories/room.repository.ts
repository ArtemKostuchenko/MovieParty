...
class RoomRepository {
    ...
    async createRoom(roomData: Room): Promise<Room> {
        ...
        if ((roomData.public !== undefined && !roomData.public) && !roomData.password) {
    ...
    async getRoomById(roomId: string, userId: string): Promise<Room> {
        const room = await RoomModel.findOne({ _id: roomId, invitedUsers: userId }).populate('videoContentId', 'soundTracks');
    ...
    async getRoomByInviteCode(inviteCode: string | undefined): Promise<Room> {
        ...
        const room = await RoomModel.findOne({ inviteCode }).select('title public users').populate('users', 'email nickname avatarURL');
    ...
    async inviteUser(roomId: string, userId: string, password: string): Promise<Room> {
        ...
        if (!room.public && !room.ownerId.equals(userId)) {
            if (!password) {
        ...
        const invitedUser = room.invitedUsers.find((user) => user._id.equals(userId));
    ...
    async existUserInRoom(roomId: string, userId: Types.ObjectId): Promise<boolean> {
        const room = await RoomModel.findOne({ _id: roomId, invitedUsers: { $all: userId } });
    ...
    async updateTime(roomId: string, userId: Types.ObjectId, time: number): Promise<void> {
        const room = await RoomModel.findById(roomId);
